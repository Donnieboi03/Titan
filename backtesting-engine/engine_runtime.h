...
namespace backtest
{
    namespace user
    {
        class User
        {
            friend class backtest::runtime::EngineRuntime;
        public:
            User()
            : user_id_(INVALID_USER_ID), runtime_(nullptr), capital_(0.0), realized_pnl_(0.0), total_volume_(0.0)
            {}

            User(
                backtest::user::Strategy&& strategy, // Registered Strategy for current User
                backtest::runtime::EngineRuntime* runtime, // Refrence to Current Runtime
                UserId user_id, // Identification for User
                double initial_capital = 100000.0 // Starting Captial for User
            )
            ...
            // Order Submissions (async)
            inline bool submit_limit_order(const std::string& ticker, engine::OrderSide side, double price, double quantity);
            inline bool submit_market_order(const std::string& ticker, engine::OrderSide side, double quantity);
            inline bool submit_cancel_order(const std::string& ticker, engine::OrderId order_id);
            inline bool submit_edit_order(const std::string& ticker, engine::OrderId order_id, double new_price, double new_quantity);
            ...
            // User Managment
            inline std::vector<engine::OrderId> get_positions(const std::string& ticker) const;  // ADD THIS
    ...
    namespace runtime
    {
        ...
        public:
            ...
            // Order submission (aync - bool for success/failure)
            bool submit_limit_order(const std::string& ticker, engine::OrderSide side, double price, double qty, user::UserId user_id = user::INVALID_USER_ID);
            bool submit_market_order(const std::string& ticker, engine::OrderSide side, double qty, user::UserId user_id = user::INVALID_USER_ID);
        ...
        private:
            ...
            std::size_t runtime_batch_size_; // Runtime-enforced batch size for flushing jobs to workers
    ...
    namespace user
    {
        inline bool User::submit_limit_order(const std::string& ticker, engine::OrderSide side, double price, double quantity)
        {
            return runtime_->submit_limit_order(ticker, side, price, quantity, user_id_);
        }

        inline bool User::submit_market_order(const std::string& ticker, engine::OrderSide side, double quantity)
        {
            return runtime_->submit_market_order(ticker, side, quantity, user_id_);
        }

        inline bool User::submit_cancel_order(const std::string& ticker, engine::OrderId order_id)
        {
            return runtime_->submit_cancel_order(ticker, order_id, user_id_);
        }

        inline bool User::submit_edit_order(const std::string& ticker, engine::OrderId order_id, double new_price, double new_quantity)
        {
