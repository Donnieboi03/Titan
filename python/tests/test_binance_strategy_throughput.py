...
runtime = tc.EngineRuntime.get_instance(
    num_threads=NUM_WORKERS, capacity=CAPACITY, verbose=False, quantum=QUANTUM
)
...
def make_strategy(state_dict):
    def strategy(user: tc.User):
        # Drive submissions purely by an internal tick counter
        ...
        for i in range(n):
            ...
            user.submit_limit_order(TICKER, side, float(price), 0.001)
        state_dict["tick"] = t + 1
    return strategy

print("2) Registering Python strategy (called by C++ threads)...")
runtime.register_strategy(make_strategy(state), starting_capital=1_000_000.0)
...
if DATA_FILE.endswith(".bin"):
    try:
        ...
        pass
updates, placed_total, elapsed = runtime.simulate(DATA_FILE, TICKER, TARGET_ORDERS)
